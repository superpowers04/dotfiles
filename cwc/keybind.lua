...
-- kbd.bind({ MODKEY }, "r", cwc.commit, { description = "reload configuration", group = "cwc" })
...
-- bash -c "grim '/tmp/SCR.png'; aseprite --oneframe /tmp/SCR.png"
...
-- kbd.bind({mod.ALT,mod.SHIFT}, "Print",function() 
--          local t = os.date('/tmp/%Y-%m-%d_%Hx%M.png');
--          cwc.spawn_with_shell("grim '"..t.."' -ps -e 'xclip -selection clipboard -target image/png -i $f'") end, {description = "Select window then screenshot, copy to clipboard", group = "cwc"})
-- kbd.bind({"Shift","Mod4"}, "Print",function() cwc.spawn("scrot '"..os.getenv('HOME').."/Pictures/Screenshots/%Y-%m-%d_%H.%M_$wx$h.png' -ps -e 'xclip -selection clipboard -target image/png -i $f'") end, {description = "Select window then screenshot, copy to clipboard and save to pictures folder", group = "cwc"})
-- kbd.bind({mod.SHIFT,mod.CTRL}, "Super_L",function() cwc.spawn("grim '"..os.getenv('HOME').."/Pictures/Screenshots/%Y-%m-%d_%H.%M_$wx$h.png' -ps -e 'xclip -selection clipboard -target image/png -i $f'") end, {description = "Select window then screenshot, copy to clipboard and save to pictures folder", group = "cwc"})
...
kbd.bind({ MODKEY, mod.SHIFT }, "f", function()
	local c = cwc.client.focused()
...
kbd.bind(MODKEY, "Return", function()
	cwc.spawn_with_shell("xfce4-termininal || kitty || alacritty || wezterm || xterm || st")
end, { description = "open a terminal", group = "launcher" })
kbd.bind({ MODKEY }, "F1", function()
	cwc.spawn_with_shell("vivaldi || firefox")
...
kbd.bind(mod.ALT, "Space", function()
	cwc.spawn_with_shell('amulet "/home/steph/SRC/amulet stuff/appmenu.lua"')
end, { description = "application launcher", group = "launcher" })
...
kbd.bind({}, "XF86AudioLowerVolume", function()
	local cmd = string.format("pactl set-sink-volume @DEFAULT_SINK@ %s%%", "-2")
...
kbd.bind({}, "XF86Calculator", function()
	cwc.spawn_with_shell(TERMINAL .. "-e lua ")
end)
...
local function repositionDisplays()
	cwc.timer.new(2, function()
		...
		local x,y = 0,0
		for i,v in pairs(cwc.screen.get()) do
			v:set_position(x,y)
			local g = v.geometry
			y = v.height
		end
...
local adaptive_sync = true
local screenOptsTbl = {
	['Toggle tearing']=function()
		local tearing = not cwc.screen.focused().allow_tearing
		for i,v in pairs(cwc.screen.get()) do
			v.allow_tearing = tearing
		end
		notifySend('Tearing set to ',tearing)
	end, 
	['Change focused display index']=function(a)
		a = tonumber(a or 1)
		cwc.screen.focused():set_mode_from_id(a)
		notifySend('Focused mode set to ',a)
		repositionDisplays()
	end,
	['720p']=function(a)
		cwc.screen.focused():set_custom_mode(1280, 720, 75)
		notifySend('Focused custom mode set to ',a)
		repositionDisplays()
	end,
	['1080p']=function(a)
		cwc.screen.focused():set_custom_mode(1920, 1080, 73)
		notifySend('Focused custom mode set to ',a)
		repositionDisplays()
	end,
	['set_res']=function(a)
	...
	['Change refresh rate']=function()
		...
		for i,v in pairs(cwc.screen.get()) do
			v:set_mode(v.width,v.height,rate)
		end
		notifyPrint('Refresh rate set to ',rate)
	end,
	['Change adaptive sync']=function()
		adaptive_sync = not adaptive_sync
		
		for i,v in pairs(cwc.screen.get()) do
			v:set_adaptive_sync(adaptive_sync)
		end
		notifyPrint('Adaptive sync set to ',adaptive_sync)
	end,
...
local scrOptsArr = {}
for i,v in pairs(screenOptsTbl) do scrOptsArr[#scrOptsArr+1]=i end
table.sort(scrOptsArr)
screenOptions = function(a)
	notifyPrint(a)
	local sep,value = a:match('(.-):(.+)')
	if sep then
		a = sep

		notifyPrint(a,value)
	end
	local f = screenOptsTbl[a]
